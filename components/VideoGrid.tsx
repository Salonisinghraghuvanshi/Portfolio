...
export default function VideoGrid() {
  ...
  const handleClose = useCallback(() => {
    ...
    setTimeout(() => { setOpenVideo(null); setAnimState(null) }, 400)
  }, [])
  ...
  useEffect(() => {
    if (animState === 'open' && modalVideoRef.current) {
      ...
      modalVideoRef.current.play().catch(() => {
        if (modalVideoRef.current) { modalVideoRef.current.muted = true; modalVideoRef.current.play() }
      })
    }
    if (animState === 'exiting' && modalVideoRef.current) modalVideoRef.current.pause()
  }, [animState])

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape' && openVideo) handleClose() }
      ...
      <section id="portfolio" className="vg-root">
        ...
        <div className="vg-wrap">
          ...
          <header className="vg-header">
            ...
            <h2 className="vg-title">Selected<br />Work</h2>
            <p className="vg-sub">
          ...
          <div className="vg-grid">
            {videos.map((video, i) => {
              const posClass =
                video.position === 'left'   ? 'vg-pos-left'
                : video.position === 'right' ? 'vg-pos-right'
                : 'vg-pos-center'
                ...
                >
                  ...
                  >
                    ...
                    <div className="vg-thumb">
                      ...
                      {/* play overlay */}
                      <div className={`vg-play-overlay ${hoveredId === video.id ? 'is-hovered' : ''}`}>
                      ...
                      {/* accent bar at bottom of thumb */}
                      <div className="vg-thumb-bar" style={{ background: video.accent }} />
                    </div>

                    {/* info footer */}
                    <div className="vg-info">
                      <div className="vg-info-left">
                        <span className="vg-info-accent-dot" style={{ background: video.accent }} />
                        <div>
                          <p className="vg-info-title">{video.title}</p>
                          <p className="vg-info-desc">{video.description}</p>
                        </div>
                      </div>
                      <svg className="vg-arr" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        {openVideo && (
          <div
            className={`vg-backdrop ${animState === 'exiting' ? 'vg-backdrop-out' : 'vg-backdrop-in'}`}
            onClick={handleClose}
          >
            <div
              className={`vg-modal ${
                animState === 'entering' ? 'vg-modal-entering'
                : animState === 'open'    ? 'vg-modal-open'
                : animState === 'exiting' ? 'vg-modal-exiting'
                : ''
              }`}
            ...
            >
              ...
              {/* title */}
              <div className={animState === 'exiting' ? '' : 'vg-title-in'} style={{ marginTop: '20px' }}>
...
const css = `
  ...
  /* orbs */
  .vg-orb { position:fixed;border-radius:50%;filter:blur(110px);pointer-events:none;z-index:0; }
  .vg-root .o1 { width:600px;height:600px;background:rgba(255,32,32,.055);top:-150px;right:-150px;animation:vg-drift 22s ease-in-out infinite alternate; }
  .vg-root .o2 { width:500px;height:500px;background:rgba(10,102,194,.065);bottom:-100px;left:-100px;animation:vg-drift 28s ease-in-out infinite alternate-reverse; }
  .vg-root .o3 { width:320px;height:320px;background:rgba(225,48,108,.04);top:45%;left:42%;animation:vg-drift 18s ease-in-out infinite alternate; }
  ...
  .vg-dot {
    ...
    animation:vg-blink 2.4s ease infinite;display:inline-block;
  }
  .vg-title {
    font-family:var(--fd);
  ...
  .vg-sub { font-size:14px;color:var(--muted);font-weight:300; }
  ...
  .vg-ring {
    ...
    border-radius:50%;border:1px solid var(--ac,rgba(255,255,255,.08));opacity:.09;pointer-events:none;z-index:0;
  }
  .vg-ring.r2 { bottom:-88px;right:-88px;width:230px;height:230px;opacity:.04; }
  ...
  .vg-card-wrap:hover .vg-play-btn { transform:scale(1.1); background:rgba(255,255,255,.22); }
  ...
  .vg-info {
    ...
    padding:16px 20px;gap:12px;position:relative;z-index:2;
  }
  .vg-info-left { display:flex;align-items:center;gap:12px; }
  .vg-info-accent-dot { width:7px;height:7px;border-radius:50%;flex-shrink:0; }
  .vg-info-title {
    font-family:var(--fd);font-size:12px;font-weight:600;
    letter-spacing:.06em;color:var(--text);line-height:1.3;margin:0;
  }
  .vg-info-desc {
    font-size:10.5px;color:var(--muted);font-weight:300;
    letter-spacing:.08em;margin:3px 0 0;
  }
  .vg-arr {
    color:var(--faint);transition:color .2s, transform .2s;flex-shrink:0;
  }
  .vg-card-wrap:hover .vg-arr { color:var(--ac);transform:translate(3px,-3px); }

  /* cta */
  .vg-cta-wrap { display:flex;justify-content:center; }
  .vg-cta-btn {
    display:inline-flex;align-items:center;gap:8px;
    ...
    transition:background .2s, border-color .2s, transform .28s cubic-bezier(.34,1.56,.64,1);
  }
  .vg-cta-btn:hover {
  ...
  .vg-modal-close {
    ...
    color:rgba(237,240,247,.6);background:none;border:none;cursor:pointer;
    font-family:var(--fb);font-size:11px;letter-spacing:.14em;
    transition:color .2s;
  }
  ...
  .vg-modal-video { width:100%;height:100%;object-fit:contain;display:block; }
  .vg-modal-bar { position:absolute;bottom:0;left:0;right:0;height:3px;z-index:2; }

  .vg-modal-title {
    font-family:var(--fd);font-size:14px;font-weight:600;
    letter-spacing:.06em;color:var(--text);margin:0 0 4px;
  }
  .vg-modal-desc {
    font-size:11px;color:var(--muted);letter-spacing:.1em;font-weight:300;margin:0;
  }
